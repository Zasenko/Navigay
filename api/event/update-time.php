...
$sql = "UPDATE Event SET start_date = ?, start_time = ?, finish_date = ?, finish_time = ? WHERE id = ?";
$params = [$start_date, $start_time, $finish_date, $finish_time, $event_id];
$types = "ssssi";
$stmt = executeQuery($conn, $sql, $params, $types);
if (checkInsertResult($stmt, $conn, 'Failed to update time of the event in Event table.')) {
    $conn->close();
    $json = ['result' => true];
