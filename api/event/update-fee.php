...
$is_free = isset($data["is_free"]) ? boolval($data["isFree"]) : false;
...
$sql = "UPDATE Event SET is_free = ?, tickets = ?, fee = ? WHERE id = ?";
$params = [$is_free, $tickets, $fee, $event_id];
$types = "issi";
$stmt = executeQuery($conn, $sql, $params, $types);
if (checkInsertResult($stmt, $conn, 'Failed to update fee in Event table.')) {
    $conn->close();
    $json = ['result' => true];
