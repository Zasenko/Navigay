...
$sql = "SELECT id, name, type_id, latitude, longitude, start_date, start_time, finish_date, finish_time, address, location, poster, poster_small, is_free, tags, updated_at FROM Event WHERE city_id = ? AND is_active = true AND (start_date = ? OR (finish_date IS NOT NULL AND finish_date > ? AND start_date < ?) OR (finish_date IS NOT NULL AND finish_date = ? AND finish_time IS NOT NULL AND finish_time > '10:00:00' AND start_date < ?))";

$params = [$city_id, $date, $date, $date, $date, $date];
$types = "isssss";
