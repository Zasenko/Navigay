...
protocol OrganizerDataManagerProtocol {
    var loadedOrganizers: [Organizer:PlaceItems] { get }
    var loadedComments: [Organizer:[DecodedComment]] { get }
    func addLoadedPlace(_ place: Organizer, with items: PlaceItems)
    func addLoadedComments(_ comments: [DecodedComment], for place: Organizer)
    func deleteLoadedComment(id: Int, for place: Organizer)
...
final class OrganizerDataManager: OrganizerDataManagerProtocol {
    
    // MARK: - Properties

    var loadedOrganizers: [Organizer:PlaceItems] = [:]
    var loadedComments: [Organizer : [DecodedComment]] = [:]
    
}

extension OrganizerDataManager {
    
    func addLoadedPlace(_ organizer: Organizer, with items: PlaceItems) {
        loadedOrganizers[organizer] = items
    }
    
    //ok
    func addLoadedComments(_ comments: [DecodedComment], for organizer: Organizer) {
        self.loadedComments[organizer] = comments
    }
    
    //ok
    func deleteLoadedComment(id: Int, for organizer: Organizer) {
        if var comments = loadedComments[organizer] {
            comments.removeAll { $0.id == id }
            loadedComments[organizer] = comments
        }
    }
//    
//    //ok
